...
{	
	...
	//Finalización del juego por limite de puntos e implementacion del mensaje de la tuberia con el ganador de la partida.
	int EstadoActual=0;
...
void CMundo::OnKeyboardDown(unsigned char key, int x, int y)
{
	...
	char tecla[5];
	sprintf(tecla,"%c",key);
	write(tuberiateclas,tecla,sizeof(tecla)+1);
}
...
//superior
	...
	proyeccion=(char*)mmap(NULL,sizeof(MemComp),PROT_WRITE|PROT_READ,MAP_SHARED,file,0); //Asignar al puntero de proyeccion el lugar donde esta proyectado el fichero
	close(file); //Cerrar fichero
	pMemComp=(DatosMemCompartida*)proyeccion; //Asignar valor del puntero de proyeccion al puntero de datos
